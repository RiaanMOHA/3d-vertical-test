"""
Bake PBR maps for the bbq-test propane-tank painted-steel material.

Used on the 20-lb cylinder visible inside the open right cabinet door.
Painted steel cylinder with mild weathering — pale silver-grey, smooth
but slightly imperfect, faint horizontal seam.

Procedural recipe:
  - Pale silver-grey base (#b6b8bc)
  - Subtle scuff / wear mottling from low-frequency Noise
  - Tiny micro-scratches from a high-frequency Wave texture

Outputs → bbq-test-textures/propane/propane_{albedo,normal,roughness,ao}.png
"""
...
def make_propane_material(name='propane_steel'):
    ...
    noi_lg.inputs['Scale'].default_value = 1.8
    noi_lg.inputs['Detail'].default_value = 3.0
    noi_lg.inputs['Roughness'].default_value = 0.55
    nt.links.new(tc.outputs['UV'], noi_lg.inputs['Vector'])

    # Tiny micro-scratch (fine wave)
    ...
    # Albedo: pale silver-grey, slight darker patches
    ramp = nt.nodes.new('ShaderNodeValToRGB')
    ramp.color_ramp.elements[0].position = 0.30
    ramp.color_ramp.elements[0].color = (0.65, 0.66, 0.68, 1)
    ramp.color_ramp.elements[1].position = 0.75
    ramp.color_ramp.elements[1].color = (0.74, 0.75, 0.77, 1)
