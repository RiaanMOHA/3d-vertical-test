...
import {
  COLOR, BED, DESK, SHELF, CHAIR, PENDANT, DOOR, DOOR_OPEN_RAD,
} from './constants.js';
...
export function buildFurniture(scene) {
  ...
  // ---- DOOR (openable): panel + handle, hinge at corner-cabinet-entrance,
  //      rotated 90° so the door swings open into the room along cabinet-wall.
  {
    const doorGroup = new THREE.Group();
    doorGroup.position.set(0, 0, 0);
    doorGroup.rotation.y = -DOOR_OPEN_RAD;

    const doorMat = new THREE.MeshStandardMaterial({
      color: 0xf2ece4, roughness: 0.6,
    });
    ...
    // local +x is door length; hinge at local x=0; panel occupies z=[-T..0]
    // so that after rotation.y=-π/2 it ends up at world x=[0..T] (inside room,
    // flush with cabinet-wall), z=[0..panelW] (spanning corner into room).
    ...
    // near free edge (local x=0.7), mid y, on local -z side so it sticks into
    // the room (world +x) after rotation.
    handle.position.set(panelW - 0.1, 1.0, -panelT - hD / 2);
  ...
  // ---- DESK: textured dark-wood top + 4 metal legs ----
  {
    ...
    const topMat = pbrMaterial('dark_wood', 1.2 / 1.0, 0.6 / 1.0);
    const topMesh = new THREE.Mesh(
      new THREE.BoxGeometry(w, DESK.topThick, d),
      topMat
    );
    topMesh.position.set(cx, topCenterY, cz);
    g.add(topMesh);
  ...
  // ---- SHELF: 4 metal posts + 4 textured dark-wood tiers + x-bracing ----
  {
    ...
    const shelfMat = pbrMaterial('dark_wood', 0.4, 0.4);
    for (const y of SHELF.tierYs) {
      const tier = new THREE.Mesh(
        new THREE.BoxGeometry(w, SHELF.tierThick, d),
        shelfMat
      );
      tier.position.set(cx, y, cz);
      g.add(tier);
    }
